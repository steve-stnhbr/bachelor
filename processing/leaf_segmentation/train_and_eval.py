...
#    opt = keras.optimizers.SGD(learning_rate=0.01, momentum=0.9, nesterov=True)
    ...
    if train_data is None:
        ...
        train_data = gen_dataset(train_dir, MASK_SUBDIR, batch_size=batch_size, lab=lab, input_shape=model.inputs.shape[1:3])
    if val_data is None:
        val_dir = os.path.join(data, 'val')
        val_data = gen_dataset(val_dir, MASK_SUBDIR, batch_size=batch_size, lab=lab, input_shape=model.inputs.shape[1:3])
