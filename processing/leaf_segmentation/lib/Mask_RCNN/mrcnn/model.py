...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    batch_counts = K.sum(tf.cast(tf.math.equal(rpn_match, 1), tf.int32), axis=1)
...
def create_dataset(dataset, config, shuffle=True, augment=False, augmentation=None,
                   random_rois=0, batch_size=1, detection_targets=False,
                   no_augmentation_sources=None):
    ...
    output_types = (
        (tf.float32, tf.float32, tf.int32, tf.float32, tf.int32, tf.float32, tf.bool),
        ()
    )
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        train_generator = create_dataset(train_dataset, self.config)
        # val_generator = create_dataset(val_dataset, self.config, shuffle=True,
        #                                batch_size=self.config.BATCH_SIZE)
        val_generator = create_dataset(val_dataset, self.config)
